...
def generate_previews(df, n=4, prefix="img", synth_df=None):
    ...
    for idx, row in samples.iterrows():
        ...
        img_resized  = cv2.resize(img_array, (int(w * scale), int(h * scale)),
        ...
        # --- Matching synthetic preview (only if synth_df was passed) ---
        if synth_df is not None:
            # Find synth rows that contain the real stem in their filepath
            ...
            if len(matching) > 0:
                ...
                s_resized = cv2.resize(s_img, (int(sw * s_scale), int(sh * s_scale)),
                                       interpolation=cv2.INTER_LINEAR)
...
def download_results(job_id):
    ...
    # Write zip to a temporary file and stream it to the browser
    zip_buffer = os.path.join(OUTPUT_FOLDER, f"{job_id}.zip")
    try:
        shutil.make_archive(
            base_name=os.path.join(OUTPUT_FOLDER, job_id),  # output path without .zip
            format='zip',
